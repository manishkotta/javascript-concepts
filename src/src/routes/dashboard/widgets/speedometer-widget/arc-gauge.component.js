...
let ArcGauge = React.createClass({
  ...
  renderArcGauge() {
    ...
    let minTextDx = this.state.radius * (1 - (this.state.min.toString().length * 0.035));
    ...
    let maxTextDx = this.state.radius * (1 - (this.state.max.toString().length * 0.02));
