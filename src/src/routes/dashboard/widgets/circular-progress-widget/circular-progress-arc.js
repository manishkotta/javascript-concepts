...
class CircularArc extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            backgroundColor: this.props.widgetBody.backgroundColor,
    ...
    renderCircularArc() {
        ...
        var arc = d3.svg.arc()
            .startAngle(0)
            .innerRadius(radius)
            .outerRadius(radius - this.state.borderWidth);
        ...
        var foreground = meter.append('path')
            ...
            .attr('fill', this.state.widgetBody.backgroundColor)
            .attr('fill-opacity', 1)
            .attr('stroke', this.state.widgetBody.backgroundColor)
...
//Denominator disabled condition check for displayformats, displayFormat.id = 2 for Percentage, displayFormat.id = 3 for Time,displayFormat.id = 4 for Text
        if (this.state.showMaxValueOnWidget == true && this.state.appliedSettings && this.state.appliedSettings.dataMetrics
            && this.state.appliedSettings.dataMetrics.displayFormat && (this.state.appliedSettings.dataMetrics.displayFormat.id == 1)) {
    ...
    render() {
        ...
        return (
            <div id={`circular-widget-${this.props.id}`}  style={{height: `${this.state.height}px`}} />
        );
    }
}

CircularArc.propTypes = {
    // percentage: PropTypes.number,
    // strokeWidth: PropTypes.number,
    // borderStyles: PropTypes.object,
    borderWidth: PropTypes.number,
    max: PropTypes.number,
};

CircularArc.defaultProps = {
    // strokeWidth: 8,
    backgroundColor: {
