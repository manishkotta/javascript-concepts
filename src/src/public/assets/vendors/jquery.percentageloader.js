...
(function () {
    ...
    window.PercentageLoader = function (el, params) {
        ...
        var settings, canvas, percentageText, valueText, items, i, item, selectors, s, ctx, progress,
            ...
            applyAngle, drawLoader, clipValue, outerDiv, ready, plugin, selectiongrad;
        ...
        selectiongrad = ctx.createLinearGradient(cX, 0, cX, canvas.height);
        // selectiongrad.addColorStop(0, '#17dfdd');
        // selectiongrad.addColorStop(0.5, '#d1c0ca');
        // selectiongrad.addColorStop(1, '#f18376');

        selectiongrad.addColorStop(0, params.grad1);
        selectiongrad.addColorStop(0.5, params.grad2);
        selectiongrad.addColorStop(1, params.grad3);

        /* Create inner gradient for the outer ring */
        innerGrad = ctx.createLinearGradient(cX, cX * 0.133333, cX, canvas.height - cX * 0.133333);
        innerGrad.addColorStop(0, '#dac2fc');
        innerGrad.addColorStop(1, '#9cc5fd');
        //innerGrad.addColorStop(2, 'red');

        /* Tube gradient (background, not the spiral gradient) */
        tubeGrad = ctx.createLinearGradient(cX, 0, cX, canvas.height);
        tubeGrad.addColorStop(0, '#dcf7fd');
        tubeGrad.addColorStop(1, '#dcf7fd');
        ...
        * than true / false */
        ...
        drawLoader = function () {
            ...
            ctx.strokeStyle = "#00acd7";
            ctx.fill();
            ctx.arc(cX, cY, innerRadius - 10, 0, Math.PI * 2, counterClockwise);
            ctx.fill();
            ...
            ctx.fill();
            ctx.arc(cX, cY, innerRadius - 12, 0, Math.PI * 2, counterClockwise);
            ctx.fill();
            // ctx.strokeStyle = '#b2d5edaa';
            ctx.stroke();
            ...
            ctx.fillStyle = selectiongrad;
            ctx.beginPath();
            // ctx.strokeStyle = 'white';
            ctx.arc(cX, cY, radius, 0, Math.PI*1, counterClockwise);
            ctx.fill();
            ctx.stroke();

            //ctx.drawImage(selectiongrad, 0, 0, canvas.width, canvas.height);
